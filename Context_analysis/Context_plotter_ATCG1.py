...
def main():
    input_dir = "/Users/odedkushnir/Projects/fitness/AccuNGS/190627_RV_CV/ATCG1/"
    ...
    sample_order = ["ATCG1-RV-minus", "ATCG1-RV-plus"]
...
# Transitions mutations plot
    ...
    transiotion_plot.set_title("Transitions variant frequency in ATCG1")
    transiotion_plot.legend(loc="upper right")

    plt.savefig(output_dir + "Transitions_variant_frequency_in_ATCG1.png", dpi=300)
    plt.close()
...
# facet plot for all mutations
    ...
    cat_plot.fig.suptitle("Transitions variant frequency in ATCG1", y=0.99)
    # cat_plot.legend(loc="upper right")

    plt.savefig(output_dir + "Facet_Transitions_variant_frequency_in_ATCG1.png", dpi=300)
    plt.close()
...
#     silent_plot.set_title("Transitions variant frequency in ATCG1, only silent mutations")
#     silent_plot.legend(loc="upper right")
#     plt.tight_layout()
#     plt.savefig(output_dir + "Silent_Transition_variant_frequency_in_ATCG1.png", dpi=300)
#     plt.close()
    ...
    type_order = ["Silent", "Missense", "NonCodingRegion"]
...
# 5’ neighbors preferences
    ...
    # context_plot.set(ylabel='Frequency')
    # context_plot.set(title="RV in cell culture 5’ neighbors preferences")
    # context_plot._legend.loc("upper right")
    plt.savefig(output_dir + "2_plots_ADAR_Context.png", dpi=300)
    plt.close()


    context_plot = sns.boxplot(x="Prev", y="Frequency", hue="label", data=data_context,
                               palette="Set2", order=context_order, hue_order=sample_order, flierprops={"marker": "."})
    sns.stripplot(x="Prev", y="Frequency", hue="label", data=data_context,
                               order=context_order, hue_order=sample_order, ax=context_plot, color="0.3", dodge=True)
    context_plot.set_yscale('log')
    context_plot.legend().set_title('')
    context_plot.set_xlabel('')
    # context_plot.set_ylabel('Frequency')
    context_plot.set_title("ATCG1 in cell culture 5’ neighbors preferences")
...
# anti-sense neighbors preferences
    ...
    context_UC_plot = sns.boxplot(x="Next", y="Freq", hue="label",
                                  data=data_context_UC,
    ...
    context_UC_plot.set_title("ATCG1 in cell culture anti-sense neighbors preferences")
