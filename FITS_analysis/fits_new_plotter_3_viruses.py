...
def main():
    ...
    rv_mutation_data = post_data_mutation(input_dir + "/AccuNGS/190627_RV_CV/RVB14/fits/output/mutation/%s" % passages)
    ...
    output_dir = input_dir + "/CirSeq/PV/OPV/fits/output/fitness/syn_plots"
    try:
        os.mkdir(output_dir)
    except OSError:
    ...
    all_data = pd.concat([rv_mutation_data, cv_mutation_data, opv_mutataion_data], sort=False)
    # print(all_data.to_string())
    all_data = all_data.rename(columns={"allele0_1": "Mutation rate"})
    ...
    all_data["Mutation"] = all_data["Mutation"].apply(lambda x: "A>G\n" + x + "-like" if len(x)>3 else x)
    ...
    g1 = sns.boxenplot(x="Mutation", hue="Virus", y="Mutation rate", data=all_data,
                       order=mutation_order, palette="Set2")
    # add_stat_annotation(g1, data=all_data, x="Mutation", y="Mutation rate", hue="Virus", order=mutation_order,
    #                     boxPairList=[(("A>G\nadar-like", "RVB14"), ("A>G\nadar-like", "CVB3")), (("A>G\nadar-like", "RVB14"),("A>G\nadar-like", "OPV"))
    #                     ,(("A>G", "RVB14"), ("A>G", "CVB3")), (("A>G", "RVB14"), ("A>G", "OPV")),
    #                                  (("A>G\nnonadar-like", "RVB14"), ("A>G\nnonadar-like", "CVB3")), (("A>G\nnonadar-like", "RVB14"),("A>G\nnonadar-like", "OPV")),
    ...
    # add_stat_annotation(g1, data=all_data, x="Mutation", y="Mutation rate", hue="Virus", order=mutation_order,
    #                     boxPairList=[(("A>G", "RVB14"), ("U>C", "RVB14")), (("A>G", "RVB14"),("G>A", "RVB14"))
    #                     ,(("A>G", "RVB14"), ("C>U", "RVB14"))],
    #                     test='Mann-Whitney', textFormat='star', loc='inside', verbose=2)
    ...
    # g1.legend(bbox_to_anchor=(1.05, 0.5), loc="upper right", borderaxespad=0.)
    plt.tight_layout()
    # plt.show()
    plt.savefig(output_dir + "/201909012_posterior_dist.png", dpi=300)
    plt.savefig(
        "/Users/odedkushnir/Google Drive/Studies/PhD/MyPosters/20190924 GGE/plots//201909012_posterior_dist.png",
        dpi=300)
    ...
    opv_fitness_data = opv_fitness_data.rename(columns={"pos": "Pos"})
    opv_fitness_data = opv_fitness_data.rename(columns={"inferred_w": "Fitness"})
    opv_fitness_data["Protein"] = np.where(opv_fitness_data["Pos"] <= 748, "5'UTR",
                                np.where(opv_fitness_data["Pos"] <= 954, "VP4",
                                    np.where(opv_fitness_data["Pos"] <= 1767, "VP2",
                                        np.where(opv_fitness_data["Pos"] <= 2481, "VP3",
                                            np.where(opv_fitness_data["Pos"] <= 3384, "VP1",
                                                np.where(opv_fitness_data["Pos"] <= 3831, "2A",
                                                    np.where(opv_fitness_data["Pos"] <= 4122, "2B",
                                                        np.where(opv_fitness_data["Pos"] <= 5109, "2C",
                                                            np.where(opv_fitness_data["Pos"] <= 5370, "3A",
                                                                np.where(opv_fitness_data["Pos"] <= 5436, "3B",
                                                                   np.where(opv_fitness_data["Pos"] <= 5985, "3C",
                                                                    np.where(opv_fitness_data["Pos"] <= 7368, "3D", "3'UTR"))))))))))))
    ...
    fitness_data = pd.concat([rv_fitness_data, cv_fitness_data, opv_fitness_data], sort=False)
    ...
    g2.savefig(output_dir + "/20190912_DFE_RVB14_CVB3_OPV.png", dpi=300)
    plt.close()
    ...
    g3.savefig(output_dir + "/20190912_Mutation_Pos_RVB14_CVB3_OPV.png", dpi=300)
    plt.close()

    g3 = sns.FacetGrid(fitness_data, col="Mutation", hue="Virus", col_order=mutation_order, palette="Set2", col_wrap=3)
    g3 = (g3.map(plt.scatter, "Pos", "Fitness", **kws).add_legend())
    # g3.fig.suptitle(" RVB14 and CVB3 (%s)" % passages, fontsize=16, weight='bold', ha='center', va='bottom')
    g3.savefig(output_dir + "/20190912_Mutation_Pos_RVB14_CVB3_OPV_in_one_plot.png", dpi=300)
    plt.close()
    ...
    g4.savefig(output_dir + "/20190912_Mutation_Protein_RVB14_CVB3_OPV_box.png", dpi=300)
    plt.close()
    ...
    plt.savefig(output_dir + "/20190912_Mutation_Protein_RVB14_CVB3_OPV_box_stat_3C.png", dpi=300)
    plt.savefig("/Users/odedkushnir/Google Drive/Studies/PhD/MyPosters/20190924 GGE/plots/20190912_Mutation_Protein_RVB14_CVB3_OPV_box_stat_3C.png", dpi=300)
    plt.close()
    ...
    g5.savefig(output_dir + "/20190912_qqplot_RVB14_CVB3_OPV.png", dpi=300)
    plt.close()
    ...
    # plt.tight_layout()
    plt.savefig(output_dir + "/20190912_KDE_RVB14_CVB3_OPV.png", dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir + "/20190912_KDE_Mutations.png", dpi=300)
