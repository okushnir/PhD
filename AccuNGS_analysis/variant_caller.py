...
def learn_gammas(data):
    ...
    for name, group in grouped:
        ...
        gamma_distributions.append(
            [name, "all", shape, scale, stats.gamma.ppf(0.95, shape, loc, scale), group["adjusted_freq"].max(),
             group["adjusted_freq"].mean(), group["adjusted_freq"].median()])
    ...
    for name, group in grouped:
        shape, loc, scale = fit_gamma_iterative(name, group)
        gamma_distributions.append(
            ["CT", "X" + name, shape, scale, stats.gamma.ppf(0.95, shape, loc, scale), group["adjusted_freq"].max(),
             group["adjusted_freq"].mean(), group["adjusted_freq"].median()])
    ...
    for name, group in grouped:
        shape, loc, scale = fit_gamma_iterative(name, group)
        gamma_distributions.append(
            ["GA", name + "X", shape, scale, stats.gamma.ppf(0.95, shape, loc, scale), group["adjusted_freq"].max(),
             group["adjusted_freq"].mean(), group["adjusted_freq"].median()])
...
def output_to_file(data, output_file):
    ...
    dataToOutput["SNP_Profile"] = dataFiltered["Ref"] + ":" + dataFiltered["major_read_count"].astype(np.int32).astype(
        str) + " " + dataFiltered["Base"] + ":" + dataFiltered["counts_for_position_x"].astype(np.int32).astype(str)
...
def main(args):
    ...
    mutation_for_rna = ["AG"]
    toPlot = [label_sample, label_control]


    toFilter = []
    dataForPlotting = data
    (data["control"].isin(toPlot)) & (data["mutation_type"].isin(mutation_for_rna)) & (data["Rank"] != 0) & (data["Pos"].isin(toFilter))]

    ax = sns.boxplot(x=orientation, y="Freq", data=dataForPlotting, whis=np.inf, boxprops=dict(alpha=.7),
                     order=["GA", "GG", "GT", "GC"], color=sns.color_palette("Set2", 10)[7])
    # sns.stripplot(x=orientation, y="Freq", data=dataForPlotting, jitter=True, dodge=True, ax=ax,
    #               palette=[".3" for x in range(16)], size=2, order=["GA", "GG", "GC", "GT"]).set_xticklabels(
    #     ["GpA", "GpG", "GpC", "GpT"], fontdict={"fontsize": 13})
    ax.set(yscale="log")
    ax.set_ylim(1 * 10 ** -6, 1)
    ax.set_ylabel("Variant frequency", fontsize=18)
    ax.set_xlabel("Dinucleotide context", fontsize=18)
    ax.tick_params(labelsize=15)
    plt.tight_layout()
    sns.despine()
    for save_format in save_formats:
        plt.savefig(output_path + "FigGA-CtrlRNA-vs-realRNA-" + orientation + ".box." + save_format, dpi=680,
                    format=save_format)
    plt.close()
