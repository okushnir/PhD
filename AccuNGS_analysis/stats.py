...
def sts_fits(file_path, new_file, skiprows=6):
    with open(file_path, "r") as stsfile, open(new_file, 'w') as outfile:
        for line in stsfile:
            new_line = line.replace(": Mann-Whitney-Wilcoxon test smaller with Bonferroni correction", "").replace("P_val:", "").replace("U_stat=", ", ")
            outfile.write(new_line)
    outfile.close()
    sts_df = pd.read_csv(new_file, skiprows=skiprows, header=None)
    ...
    #                                           np.where((sts_df["p-value"] >= 0.0001) & (sts_df["p-value"] <= 0.001), "***",
    #                                                    np.where(sts_df["p-value"] <= 0.0001, "****", "ns"))))
    sts_df["Test"] = "Mann-Whitney-Wilcoxon test smaller with Bonferroni correction"
    sts_df.drop("stat", axis=1, inplace=True)
...
def main():
    ...
    input_dir = "D:/My Drive/Studies/PhD/Thesis/FinalVer/AfterReview/20220921_fits_syn_plots"
    sts_file = input_dir + "/sts.csv"
    new_file = input_dir + "/sts_edited.csv"
    sts_fits_df = sts_fits(sts_file, new_file)
    sts_fits_df.to_csv("D:/My Drive/Studies/PhD/Thesis/FinalVer/AfterReview/20220921_fits_syn_plots/sts_fits_df.csv")
