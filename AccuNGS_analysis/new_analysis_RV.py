...
def main():
    ...
    output_dir = input_dir + "/20201012_without_pass_0_passage_plots"
    try:
        os.mkdir(output_dir)
    except OSError:
    ...
    data_miseq = data_filter

    data_miseq = data_miseq[data_filter["label"] != "RNA Control_RND"]
    ...
    label_order = ["p2-1", "p2-2", "p2-3", "p5-1", "p5-2", "p5-3", "p8-1",
                   "p8-2", "p8-3", "p10-2", "p10-3", "p12-1", "p12-2", "p12-3"] #"RNA Control_RND", "RNA Control_Primer_ID",
    ...
    data_miseq_ag_pass5 = data_miseq_ag.loc[data_miseq_ag.passage == 5]
    ...
    plt.close()
    data_miseq["passage"] = data_miseq["passage"].astype(str)
    g2 = sns.catplot(x="passage", y="frac_and_weight", data=data_miseq, hue="Mutation", order=miseq_order, palette="tab20"
    ...
    # plt.close()
    #
    # g9 = sns.catplot("label", "frac_and_weight", data=data_miseq_uc, hue="Next", order=label_order, palette="tab20",
    ...
    # plt.close()
    #
    # data_miseq_ag_grouped = data_miseq_ag.groupby(["ADAR_like", "label", "Type", "Pos", "Protein", "passage"])["frac_and_weight"].agg(lambda x: weighted_varaint(x))
    ...
    #
    # data_miseq_ag_grouped = data_miseq_ag_grouped[data_miseq_ag_grouped["label"] != "RNA Control_Primer_ID"]
    # data_miseq_ag_grouped = data_miseq_ag_grouped[data_miseq_ag_grouped["label"] != "RNA Control_RND"]
