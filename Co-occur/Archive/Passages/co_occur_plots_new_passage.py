...
def main():
    ...
    for replica in replica_lst:
        ...
        df = df.merge(data_mutation, how="outer", on=["Pos", "label", "Rank"])
        ...
        df_co_occur_new = df_co_occur_new.rename(columns={"Frequency_x": "Frequency", "Base_x": "Base", "Ref_x": "Ref"})
        ...
        df_grouped["Next_1line_Pos"] = df_grouped["Pos"].shift(periods=-1)
        df_grouped = df_grouped.fillna("0, ")
        df_grouped["Next_1line_Pos_filter"] = df_grouped.apply(
            lambda x: compare_2_strings(x["Pos"], x["Next_1line_Pos"]), axis=1)
        df_grouped = df_grouped.rename_axis('index1').reset_index()
        df_grouped["index1"] = df_grouped.apply(lambda x: np.where(x.Next_1line_Pos_filter != [], x.index1 + 1, x.index1), axis=1)
        # df_grouped["index1"] = df_grouped["index1"].astype(str)
        ...
        df_co_occur_merge2.to_csv(input_dir + "/all_co_occur_merge_replica%s.csv" % replica, sep=",", encoding='utf-8')
