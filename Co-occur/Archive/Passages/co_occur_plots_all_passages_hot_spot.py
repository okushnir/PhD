...
def main():
    ...
    output_dir = input_dir + "/20201222_co_occur_plots"
    try:
        os.mkdir(output_dir)
    except OSError:
    ...
    data_replica1 = pd.read_csv(input_dir + "/all_co_occur_merge_replica1.csv")
    data_replica2 = pd.read_csv(input_dir + "/all_co_occur_merge_replica2.csv")
    data_replica3 = pd.read_csv(input_dir + "/all_co_occur_merge_replica3.csv")

    data_all = pd.concat([data_replica1, data_replica2, data_replica3])
    print(data_all.to_string())

    data_all = data_all.sort_values(by=["Pos", "label", "Stretch"])
    data_all.to_csv(input_dir + "/all_co_occur_merge_all_replicas.csv", sep=",", encoding='utf-8')

    grouped2 = data_all.groupby(["Pos"])
    ...
    df_grouped["first_Pos"] = df_grouped["Pos"].apply(lambda x: int(str(x).split(",")[0]))
    df_grouped["Next_line_last_Pos"] = df_grouped["Next_line_Pos"].apply(lambda x: int(str(x).split(",")[-1]))


    df_grouped["Next_line_Pos_intercept"] = df_grouped.apply(
        lambda x: compare_2_strings(x["Pos"], x["Next_line_Pos"]), axis=1)
    df_grouped["Next_line_Pos_intercept"] = np.where(df_grouped["Next_line_Pos_intercept"] == "", 0, df_grouped["Next_line_Pos_intercept"])

    df_grouped = df_grouped.rename_axis('index1').reset_index()
    df_grouped = df_grouped.fillna("0")


    df_grouped["index1"] = df_grouped.apply(lambda x: np.where((x.Next_line_last_Pos-x.first_Pos < 150) & (x.Next_line_Pos_intercept != 0), x.index1+1, x.index1), axis=1)


                                            #                    x.index1 + 5,
                                            #                    np.where(x.Next_5line_Pos_filter != [], x.index1 + 5,
                                            #                             np.where(x.Next_4line_Pos_filter != [], x.index1 + 4,
                                            #                                      np.where(x.Next_3line_Pos_filter != [], x.index1 + 3, np.where(x.Next_2line_Pos_filter != [], x.index1 + 2, np.where(x.Next_1line_Pos_filter != [], x.index1 + 1, x.index1)))))),
                                            # axis=1)
    # df_grouped["Next_1line_index1"] = df_grouped["index1"].shift(periods=-1)
    # df_grouped = df_grouped.fillna(0)
    # df_grouped["index1"] = df_grouped.apply(lambda x: np.where(x.index1 >= x.Next_1line_index1, x.index1, x.Next_1line_index1), axis=1)
    # df_grouped["index1"] = df_grouped["index1"].astype(int)
    # df_grouped["Next_1line_index1"] = df_grouped["index1"].shift(periods=-1)
    # df_grouped["Prev_1line_index1"] = df_grouped["index1"].shift(periods=+1)
    # df_grouped["index1"] = df_grouped.apply(
    #     lambda x: np.where(x.Next_1line_index1 == x.Prev_1line_index1, x.Next_1line_index1, x.index1), axis=1)
    # df_grouped["index1"] = df_grouped["index1"].astype(int)
