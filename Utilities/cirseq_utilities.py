...
def find_mutation_type(freqs_file, ncbi_id):
    ...
    for kmer in range(data.first_valid_index(), (len(data)), 12):
        ...
        kmer_df['Type'] = kmer_df[['Consensus_aa', 'Mutated_aa']].apply(
    ...
    lower_data["adj_freq"] = ""
    lower_data["prev1base"] = ""
    lower_data["next1base"] = ""
    ...
    data_final = data_final[["Pos", "Base", "Freq", "Ref", "Read_count", "Rank", "Prob",
                                    ...
                                    "prev1base", "next1base", "prev2bases", "next2bases", "Consensus_codon",
                                    "Mutated_codon", "Codon", "Consensus_aa", "Mutated_aa", "Type"]]
...
def translate_codon(data):
    ...
    data['Consensus_aa'].loc[data.first_valid_index():data.first_valid_index()+11] = protein_ref
    data['Consensus_aa'].replace('(', '', inplace=True)
    data['Consensus_aa'].replace(')', '', inplace=True)

    data["Mutated_aa"] = data["Codon"].apply(translate)
    data["Mutated_aa"].replace('(', '', inplace=True)
    data["Mutated_aa"].replace(')', '', inplace=True)
    return data
...
def filter_by_coverage_mutation(type_file, output_dir, min_read_count=None):
    ...
    data = data[["Pos", "Base", "Freq", "Ref", "Read_count", "Rank", "Prob", "Codon", "Consensus_aa", "Mutated_aa", "Type",
...
def main():
    # freqs_file = "/Users/odedkushnir/Projects/fitness/test/SRR445754.freqs"
    # data = find_mutation_type(freqs_file, "JQ747745")
    # # data = add_mutation_type_to_df(data, "JQ747745")
    # data.to_csv("/Users/odedkushnir/Projects/fitness/test/data_cirseq.csv", sep=',', encoding='utf-8')
    # print(data)
    freqs = freqs_to_dataframe("/Users/odedkushnir/Projects/fitness/AccuNGS/RV/RV-Plasmid/q30_3UTR_new/RV-Plasmid.with.mutation.type.freqs")
    freqs["filter"] = freqs["Pos"]>3605
    freqs = freqs[freqs["filter"] == True]
    freqs.to_csv("/Users/odedkushnir/Projects/fitness/AccuNGS/RV/RV-Plasmid/q30_3UTR_new/RV-Plasmid.with.mutation.type.freqs.new", sep='\t', encoding='utf-8')
